...
include_once("objects/megusta/MegustaManager.php");
$statusRet  = 'ERROR';
$mensajeRet = 'ERROR';
$sesionManager = new SesionManagerImpl();
$objPrincipalManager = new MegustaManager();
...
if (sizeof($_POST) > 0) {
    ...
    if (preg_match('/^(alta|editar|listar|get|eliminar)$/i', $var_accion)) {
        if ($var_accion == 'alta') {
            $objPrincipalManager->agregarMegusta($_POST);
        } else if ($var_accion == 'editar') {
            $objPrincipalManager->modificarMegusta($_POST);
        } else if ($var_accion == 'eliminar') {
            $objPrincipalManager->eliminarMegusta($_POST);
        }
