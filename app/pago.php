...
include_once("objects/pago/PagoManager.php");
$statusRet  = 'ERROR';
$mensajeRet = 'ERROR';
$sesionManager = new SesionManagerImpl();
$objPrincipalManager = new PagoManager();
...
if (sizeof($_POST) > 0) {
    ...
    if (preg_match('/^(alta|editar|listar|get|eliminar)$/i', $var_accion)) {
        if ($var_accion == 'alta') {
            $objPrincipalManager->agregarPago($_POST);
        } else if ($var_accion == 'editar') {
            $objPrincipalManager->modificarPago($_POST);
        } else if ($var_accion == 'eliminar') {
            $objPrincipalManager->eliminarPago($_POST);
        }
