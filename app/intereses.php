...
include_once("objects/intereses/InteresesManager.php");
$statusRet  = 'ERROR';
$mensajeRet = 'ERROR';
$sesionManager = new SesionManagerImpl();
$objPrincipalManager = new InteresesManager();
...
if (sizeof($_POST) > 0) {
    ...
    if (preg_match('/^(alta|editar|listar|get|eliminar)$/i', $var_accion)) {
        if ($var_accion == 'alta') {
            $objPrincipalManager->agregarIntereses($_POST);
        } else if ($var_accion == 'editar') {
            $objPrincipalManager->modificarIntereses($_POST);
        } else if ($var_accion == 'eliminar') {
            $objPrincipalManager->eliminarIntereses($_POST);
        }
