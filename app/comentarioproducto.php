...
if (sizeof($_POST) > 0) {
    ...
    if (preg_match('/^(alta|editar|listar|get|eliminar)$/i', $var_accion)) {
        if ($var_accion == 'alta') {
            $objPrincipalManager->agregarComentarioproducto($_POST);
        } else if ($var_accion == 'editar') {
            $objPrincipalManager->modificarComentarioproducto($_POST);
        } else if ($var_accion == 'eliminar') {
            $objPrincipalManager->eliminarComentarioproducto($_POST);
        }
